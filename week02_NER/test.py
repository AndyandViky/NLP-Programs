...
train_loader = torch.utils.data.DataLoader(datasets.MNIST('./datasets/mnist', train=True, download=True,
                                                          transform=transforms.ToTensor()), batch_size=64, shuffle=True)
test_loader = torch.utils.data.DataLoader(datasets.MNIST('./datasets/mnist', train=False, download=True,
...
class ModelVAE(torch.nn.Module):
    ...
    def reparameterize(self, z_mean, z_var):
        if self.distribution == 'normal':
            ...
            p_z = HypersphericalUniform(self.z_dim - 1, device="cuda:0")
        else:
            raise NotImplemented
...
def train(model, optimizer):
    for i, (x_mb, y_mb) in enumerate(train_loader):
        ...
        x_mb = (x_mb > torch.distributions.Uniform(0, 1).sample(x_mb.shape)).float().cuda()

        _, (q_z, p_z), _, x_mb_ = model(x_mb.reshape(-1, 784))

        loss_recon = nn.BCEWithLogitsLoss(reduction='none').cuda()(x_mb_, x_mb.reshape(-1, 784)).sum(-1).mean()
...
def test(model, optimizer):
    ...
    for x_mb, y_mb in test_loader:
        ...
        print_['recon loss'].append(float(nn.BCEWithLogitsLoss(reduction='none').cuda()(x_mb_,
